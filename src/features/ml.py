...
def analyze_feature_importance_shap(model, model_type, data_sample, features, output_dir, fold_idx=None):  # pragma: no cover
    ...
    safe_makedirs_for_file(os.path.join(output_dir, "dummy.txt"))
    ...
    try:
        ...
        if model_type == "CatBoostClassifier" and CatBoostClassifier and Pool:
            ...
            # Try TreeExplainer for other model types
            try:
                explainer = shap.TreeExplainer(model)
                logging.info(f"      Calculating SHAP values for {model_type}...")
                shap_values = explainer.shap_values(X_shap)
            except Exception as tree_error:
                logging.error(f"   (Error) Error during SHAP analysis: {tree_error}")
                return
...
def build_catboost_model(params=None, use_gpu=True):
    ...
    if CatBoostClassifier is None:
        raise ImportError("CatBoostClassifier is not available. Please install catboost.")
    if params is None:
        params = {}
    params = params.copy()
    if use_gpu:
        params['task_type'] = 'GPU'
        params['devices'] = '0'  # Colab GPU is device 0
    params.setdefault('thread_count', -1)  # Use all CPU cores for preprocessing
    params.setdefault('max_ctr_complexity', 4)
...
def build_catboost_model(params: dict, use_gpu: bool = True, ram_fraction: float = 0.8, gpu_ram_fraction: float = 0.8):
    ...
    params['used_ram_limit'] = f"{ram_gb}gb"
    if use_gpu:
        params['task_type'] = 'GPU'
        params['devices'] = '0'
        # Set GPU RAM fraction via environment variable (CatBoost uses all by default)
