...
try:
    ...
    SPREAD_POINTS = trading_cost_cfg["spread_points"]
    MIN_SLIPPAGE_POINTS = trading_cost_cfg["min_slippage_points"]
    MAX_SLIPPAGE_POINTS = trading_cost_cfg["max_slippage_points"]
    POINT_VALUE = trading_cost_cfg["point_value"]
    MIN_LOT_SIZE = trading_cost_cfg["min_lot_size"]
    OMS_INITIAL_EQUITY = trading_cost_cfg["initial_equity"]
except Exception as e:
...
def run_backtest_simulation_v34(*args, **kwargs):
    """
...
def precompute_tp_array(df: pd.DataFrame) -> np.ndarray:
    ...
    src.strategy.run_backtest_simulation_v34 = run_backtest_simulation_v34
