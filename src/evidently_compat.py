...
class ProfessionalValueDrift:
    ...
    def calculate(self, reference_data: Any, current_data: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            drift_detected = any([
                ...
                drift_results.get("std_ratio", 1.0) > 2.0 or drift_results.get("std_ratio", 1.0) < 0.5  # Variance change
            ])
            ...
            logger.debug(f"📈 Professional drift calculation for {self.column_name}: score={drift_score:.3f}, detected={drift_detected}")
            return result
            
        except Exception as e:
            logger.warning(f"⚠️ Professional drift calculation error for {self.column_name}: {e}")
        ...
        try:
            ...
            # Dictionary
                ...
                if hasattr(values, "__iter__") and not isinstance(values, str):
                    return np.array([v for v in values if v is not None and not (isinstance(v, float) and np.isnan(v))])
                else:
    ...
    def _calculate_drift_metrics(self, ref_values: np.ndarray, cur_values: np.ndarray) -> Dict[str, float]:
...
__all__ = ["ValueDrift", "DataDrift", "EVIDENTLY_AVAILABLE", "initialize_evidently", "ProfessionalValueDrift"]
