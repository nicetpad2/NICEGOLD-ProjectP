...
def test_print_qa_summary_empty_df(caplog):
    ...
    # ข้อความ warning จะอยู่ใน caplog.records
    assert any("ไม่มีไม้ที่ถูกเทรด" in r.getMessage() for r in caplog.records)
...
def test_print_qa_summary_valid_df(caplog):
    ...
    # max_drawdown = (min(equity/roll_max - 1)) = 0 (no drawdown)
    assert res["max_drawdown"] == 0.0
    assert any(" =  = = QA SUMMARY = =  = " in r.getMessage() for r in caplog.records)
...
def test_convert_thai_datetime_invalid(caplog):
    ...
    assert any("ไม่สามารถแปลงปี พ.ศ." in r.getMessage() for r in caplog.records)
