...
try:
    ...
    # Import new deep understanding modules
    ...
    # Import smart monitoring modules
    ...
    from .integration import ProjectPIntegrator, PipelineMonitor, AutoImprovement
...
class AgentController:
    ...
    def __init__(self, project_root: str = None):
        ...
        logger.info(f"🤖 Enhanced AI Agent Controller initialized for: {self.project_root}")
    ...
    def run_comprehensive_analysis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            understanding_results = self.understanding_system.analyze_project_structure()
    ...
    def run_comprehensive_deep_analysis(self) -> Dict[str, Any]:
        ...
        """
        logger.info("🔍 Starting comprehensive deep analysis with enhanced capabilities...")
        ...
        comprehensive_recommendations = self.recommendation_engine.generate_comprehensive_recommendations(analysis_results)
        analysis_results['intelligent_recommendations'] = comprehensive_recommendations
        
        # Generate enhanced summary
        analysis_results['enhanced_summary'] = self._generate_enhanced_summary(analysis_results)
        analysis_results['priority_actions'] = self._extract_priority_actions(analysis_results)
        analysis_results['implementation_roadmap'] = comprehensive_recommendations.get('implementation_roadmap', {})
    ...
    def _setup_smart_monitoring(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.realtime_monitor.add_callback('threshold_breach', handle_monitoring_alert)
            self.realtime_monitor.add_callback('anomaly_detected', handle_monitoring_alert)
    ...
    def _generate_comprehensive_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if understanding:
                total_files = understanding.get('code_quality', {}).get('total_files', 0)
                if total_files > 0:
                    health_factors.append(min(100, (total_files * 2)))  # More files = better structure
            
            # Code quality factors
            if code_analysis and code_analysis.get('issues'):
                total_issues = len(code_analysis['issues'])
                critical_issues = sum(1 for issue in code_analysis['issues'] if issue.get('severity') == 'critical')
                
                if total_issues > 0:
                    quality_score = max(0, 100 - (critical_issues * 20) - (total_issues * 2))
                    health_factors.append(quality_score)
                
                summary['critical_issues'] = [
                    issue for issue in code_analysis['issues'] 
            ...
            if auto_fixes:
                ...
                fix_rate = (fixes_successful / fixes_attempted) * 100 if fixes_attempted > 0 else 0
            ...
            if optimization:
                ...
                health_factors.append(max(0, 50 + improvement_score))  # Base 50 + improvement
                
                summary['performance_gains'] = {
                    'memory_improvement': overall_improvement.get('memory_improvement', 0),
                    'performance_improvement': overall_improvement.get('performance_improvement', 0),
                    'overall_improvement': improvement_score
                }
            
            # Calculate overall health score
            if health_factors:
                summary['project_health_score'] = sum(health_factors) / len(health_factors)
            
            # Key metrics
            summary['key_metrics'] = {
                'total_files': understanding.get('code_quality', {}).get('total_files', 0),
                'total_lines': understanding.get('code_quality', {}).get('total_lines', 0),
                ...
                'optimization_opportunities': len(optimization.get('optimizations', {}))
            }
            
            # Next priorities based on findings
            if summary['critical_issues']:
                summary['next_priorities'].append("Address critical code quality issues")
            ...
            if auto_fixes.get('fixes_failed', 0) > 0:
                summary['next_priorities'].append("Review and manually fix failed auto-fixes")
    ...
    def _generate_recommendations(self, results: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if code_analysis.get('issues'):
                critical_count = len([i for i in code_analysis['issues'] if i.get('severity') == 'critical'])
                if critical_count > 0:
                    recommendations.append({
                        'priority': 'high',
    ...
    def _generate_next_steps(self, results: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            if critical_recs:
                next_steps.append("🚨 URGENT: Address critical project health issues immediately")
            ...
            if critical_issues:
                next_steps.append(f"🔧 Fix {len(critical_issues)} critical code issues identified")
            ...
            health_score = summary.get('project_health_score', 0)
            if health_score < 80:
                next_steps.append("📈 Implement performance optimizations to improve health score")
...
"""
    ...
    def auto_improve_project(self) -> Dict[str, Any]:
        ...
        opportunities = self.auto_improvement.identify_improvement_opportunities(performance)
    ...
    def _generate_enhanced_summary(self, results: Dict[str, Any]) -> str:
        ...
        if 'monitoring_status' in results:
            ...
            if monitoring.get('active_alerts'):
                summary_parts.append(f"🚨 Active alerts: {len(monitoring['active_alerts'])}")
            else:
    ...
    def _extract_priority_actions(self, results: Dict[str, Any]) -> List[str]:
        ...
        if 'performance_metrics' in results:
            ...
            if auc_mean < 0.7:
                actions.append("🎯 Priority: Improve AUC performance (current: {:.3f})".format(auc_mean))
        ...
        if 'monitoring_status' in results:
            ...
            if critical_alerts:
                actions.append(f"🚨 Urgent: Address {len(critical_alerts)} critical alerts")
        ...
        if 'improvement_opportunities' in results:
            ...
            if high_priority:
                actions.append(f"⚡ High Priority: {len(high_priority)} improvement opportunities")
    ...
    def run_projectp_pipeline(self, mode: str = "full", wait_for_completion: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if wait_for_completion:
                ...
                pipeline_results['status'] = 'completed' if result.returncode == 0 else 'failed'
            else:
    ...
    def monitor_projectp_execution(self, terminal_id: str = None, timeout: int = 3600) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if terminal_id:
                try:
                    ...
                    if output:
                        # หา AUC values
                        import re
                        auc_matches = re.findall(r'AUC[:\s]*([0-9]+\.?[0-9]*)', output)
                        if auc_matches:
                            latest_auc = float(auc_matches[-1])
                            monitoring_results['metrics_detected']['latest_auc'] = latest_auc
                        ...
                        if 'Error' in output or 'Exception' in output:
                            ...
                            monitoring_results['errors_detected'].extend(error_lines[-5:])  # เก็บ 5 ล่าสุด
                        
                        # หา warning patterns
                        if 'Warning' in output or 'WARNING' in output:
                            warning_lines = [line for line in output.split('\n') if 'Warning' in line or 'WARNING' in line]
                            monitoring_results['warnings_detected'].extend(warning_lines[-5:])  # เก็บ 5 ล่าสุด
                            
                except Exception as e:
                    monitoring_results['monitoring_errors'] = [f"Failed to get terminal output: {e}"]
            ...
            classification_report_path = os.path.join(self.project_root, 'classification_report.json')
            if os.path.exists(classification_report_path):
                try:
                    with open(classification_report_path, 'r') as f:
                        import json
                        classification_data = json.load(f)
                        monitoring_results['latest_classification_report'] = classification_data
                except Exception as e:
                    monitoring_results['warnings_detected'].append(f"Failed to read classification report: {e}")
    ...
    def get_projectp_status_summary(self) -> str:
        ...
        try:
            ...
            summary_parts.append("- agent.run_projectp_pipeline('full') - Run full pipeline")
            summary_parts.append("- agent.run_projectp_pipeline('until_auc_70') - Run until AUC ≥ 70%")
            summary_parts.append("- agent.monitor_projectp_execution() - Check current status")
    ...
    def wait_for_auc_target(self, target_auc: float = 0.7, max_attempts: int = 10, check_interval: int = 60) -> Dict[str, Any]:
        """
            ...
            try:
                ...
                if os.path.exists(classification_report_path):
                    ...
                    if current_auc >= target_auc:
                        ...
                        logger.info(f"🎉 Target achieved! AUC {current_auc:.3f} >= {target_auc}")
                        break
                else:
        ...
        if wait_results['final_status'] == 'in_progress':
            ...
            logger.warning(f"⏰ Timeout: AUC target not reached after {max_attempts} attempts")
...
def main():
    ...
    if args.action == 'analyze':
        ...
        print(f"✅ Analysis complete. Results saved to: {results.get('results_saved_to', 'Unknown')}")
