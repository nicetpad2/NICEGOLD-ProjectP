...
class AutoImprovement:
    ...
    def _read_walkforward_results(self) -> Dict[str, Any]:
        ...
        try:
            csv_path = os.path.join(
                self.project_root, "output_default", "walkforward_metrics.csv"
            )
    ...
    def _read_threshold_results(self) -> Dict[str, Any]:
        ...
        try:
            csv_path = os.path.join(
                self.project_root, "output_default", "threshold_results.csv"
            )
            if os.path.exists(csv_path):
                df = pd.read_csv(csv_path)
            ...
            if os.path.exists(json_path):
                with open(json_path, "r") as f:
                    ...
                    return {
                        ...
                        "classification_f1_macro": report.get("macro avg", {}).get(
                            "f1 - score", 0
                        ),
                        "classification_precision_macro": report.get(
                            "macro avg", {}
                        ).get("precision", 0),
                        "classification_recall_macro": report.get("macro avg", {}).get(
                            "recall", 0
                        ),
                    }
        except Exception as e:
        ...
        for opportunity in opportunities[:3]:  # ทำสูงสุด 3 improvements ต่อครั้ง
            try:
                ...
                if strategy in self.optimization_strategies:
                    improvement_result = self.optimization_strategies[strategy](
                        opportunity
                    )
    ...
    def _tune_hyperparameters(self, opportunity: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if os.path.exists(config_path):
            ...
            if "n_estimators" in config["model"]:
                config["model"]["n_estimators"] = min(
                    config["model"]["n_estimators"] * 1.5, 1000
                )
            else:
            ...
            if "learning_rate" in config["model"]:
                config["model"]["learning_rate"] = max(
                    config["model"]["learning_rate"] * 0.8, 0.01
                )
            else:
    ...
    def _optimize_thresholds(self, opportunity: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            threshold_script = os.path.join(
                self.project_root, "threshold_optimization.py"
            )
