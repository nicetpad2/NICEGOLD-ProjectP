...
def _get_projectp():
    """Get ProjectP module with lazy import to avoid circular import"""
    global _ProjectP
    if _ProjectP is None:
        try:
            _ProjectP = importlib.import_module("ProjectP")
        except ImportError as e:
            logger.warning(f"Could not import ProjectP: {e}")
            _ProjectP = None
    return _ProjectP
...
class ProjectPIntegrator:
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            try:
                self.projectp_module = _get_projectp()
                if self.projectp_module is None:
                    logger.error("Failed to load ProjectP module")
                    return False
        ...
        try:
            ...
            for func_name in functions_to_monitor:
                if hasattr(self.projectp_module, func_name):
                    ...
                    wrapped_func = self._create_monitored_function(
