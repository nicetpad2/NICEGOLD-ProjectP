...
def ensure_metrics_summary(output_dir):
    ...
    with progress:
        ...
        if metrics.get('auc', 0) < 0.6:
            progress.console.print(f"[yellow][metrics] [คำแนะนำ] AUC ต่ำ อาจ underfit หรือข้อมูลไม่พอ ลองปรับ feature/model (AUC={safe_fmt(metrics.get('auc'))})")
        if metrics.get('mcc', 0) < 0.1:
            progress.console.print(f"[yellow][metrics] [คำแนะนำ] MCC ต่ำมาก อาจมีปัญหา leakage หรือ class imbalance (MCC={safe_fmt(metrics.get('mcc'))})")
        ...
        try:
            ...
            notify_slack(f"[ProjectP] Metrics summary: AUC={safe_fmt(metrics.get('auc'))}", os.environ.get('SLACK_WEBHOOK_URL'))
            print_next_steps(metrics)
            # แจ้งเตือนผ่าน LINE Notify (option)
            line_token = os.environ.get('LINE_NOTIFY_TOKEN')
            if line_token:
                notify_line(f"[ProjectP] Metrics summary: AUC={safe_fmt(metrics.get('auc'))} | MCC={safe_fmt(metrics.get('mcc'))}", line_token)
...
def validate_metrics_and_warn(metrics: dict, context: str = ""):
    """
    ตรวจสอบ metric anomaly เช่น metric ต่ำ, class imbalance, missing key, และแจ้งเตือนด้วย rich panel
    """
    warnings = []
    # ตรวจสอบ missing key
    required_keys = ["accuracy", "precision", "recall", "auc"]
    for k in required_keys:
        if k not in metrics:
            warnings.append(f"[Metrics] Missing key: {k}")
    # ตรวจสอบ metric ต่ำผิดปกติ
    if metrics.get("auc", 1.0) < 0.6:
        warnings.append(f"[Metrics] AUC ต่ำผิดปกติ: {metrics.get('auc', 0):.3f}")
    if metrics.get("accuracy", 1.0) < 0.6:
        warnings.append(f"[Metrics] Accuracy ต่ำผิดปกติ: {metrics.get('accuracy', 0):.3f}")
    # Class imbalance
    if "support_0" in metrics and "support_1" in metrics:
        if min(metrics["support_0"], metrics["support_1"]) < 10:
            warnings.append(f"[Metrics] Class imbalance: support_0={metrics['support_0']}, support_1={metrics['support_1']}")
    # แจ้งเตือนด้วย rich panel
    if warnings:
        panel_title = f"[bold red]⚠️ Metric/ข้อมูลผิดปกติ! {context}" if context else "[bold red]⚠️ Metric/ข้อมูลผิดปกติ!"
        console.print(Panel("\n".join(warnings), title=panel_title, border_style="red"))
    return warnings
