...
def on_event(event_type, event):
    ...
    if order:
        ...
        table = Table(title=f"OMSMM {event_type.upper()}", box=box.ROUNDED, show_header=True, header_style="bold magenta")
        for k in ['id', 'symbol', 'qty', 'order_type', 'price', 'status', 'filled_qty', 'timestamp', 'user_id', 'tag']:
...
def run_backtest(config=None):
    ...
    if not os.path.exists(data_path):
        pro_log(f"[Backtest] Predictions data not found: {data_path}. Please run predict step first.", level="error", tag="Backtest")
        return None
    ...
    if missing_cols:
        pro_log(f"[Backtest] ERROR: Predictions data missing required columns: {missing_cols}", level="error", tag="Backtest")
    ...
    import psutil
    pro_log(f"[Backtest] RAM: {psutil.virtual_memory().percent:.1f}% used, CPU: {psutil.cpu_percent()}%", tag="Backtest")
    if 'label' not in df.columns:
        pro_log(f"[Backtest] ERROR: No 'label' column found in predictions data!", level="error", tag="Backtest")
        raise ValueError("No 'label' column found in predictions data!")
    if df.isnull().sum().sum() > 0:
        pro_log(f"[Backtest] WARNING: Missing values detected in predictions data!", level="warn", tag="Backtest")    # --- เทพ: Resource Allocation 80% ---
    ...
    elif isinstance(result, dict) and 'df' in result and hasattr(result['df'], 'to_csv'):
        result['df'].to_csv(out_path, index=False)
    pro_log(f"[Backtest] Saved backtest result to {out_path}", level="success", tag="Backtest")
    ...
    try:
        ...
        if df_result is not None:
            ...
            summary_path = os.path.join("output_default", "backtest_summary_metrics.json")
            with open(summary_path, "w", encoding="utf-8") as f:
                json.dump(summary, f, indent=2)
            pro_log(f"[Backtest] Summary metrics exported to {summary_path}", tag="Backtest")
    except Exception as e:
        pro_log(f"[Backtest] Summary metrics export error: {e}", level="warn", tag="Backtest")
        ...
        if df_result is not None:
            for col in ['equity', 'pnl', 'balance']:
                if col in df_result.columns:
                    ...
                    plt.savefig(os.path.join("output_default", f"backtest_{col}_curve.png"))
                    plt.close()
                    pro_log(f"[Backtest] {col.capitalize()} curve exported.", tag="Backtest")
    except Exception as e:
        pro_log(f"[Backtest] Equity/PNL curve export error: {e}", level="warn", tag="Backtest")
    ...
    pro_log(f"[Backtest] Diagnostics log exported to {diagnostics_path}", tag="Backtest")
    ...
    try:
        ...
        pro_log(f"[Backtest] Output validation error: {e}", level="error", tag="Backtest")
    ...
    with open(perf_log_path, "a", encoding="utf-8") as f:
        f.write(f"Backtest finished at {pd.Timestamp.now()} | Duration: {end_time - start_time:.2f} sec\n")
    pro_log(f"[Backtest] Performance log exported to {perf_log_path}", tag="Backtest")

    # --- เทพ: Usability hints ---
    pro_log(f"[Backtest] ผลลัพธ์หลัก: {out_path}\nดู resource log ที่ {resource_log_path}\nดู summary metrics ที่ {summary_path if 'summary_path' in locals() else '-'}\nดู diagnostics log ที่ {diagnostics_path}", tag="Backtest")
    pro_log_json({"event": "end_backtest", "result": str(out_path)}, tag="Backtest", level="SUCCESS")
    ...
    for i, row in df.iterrows():
        ...
        if row['target'] == 1:
            oid = omsmm_engine.send_order(symbol='XAUUSD', qty=1, order_type='MARKET', price=row['Close'])
            if oid:
                omsmm_engine.fill_order(oid)
...
def execution_worker():
    while True:
        ...
        # จำลอง response:
        response = {'order_id': order_req['order_id'], 'status': 'FILLED', 'fill_qty': order_req['qty'], 'price': order_req['price']}
...
def run_oms_execution_integration():
    ...
    try:
        ...
        pro_log(f"[Backtest] Error loading main training data: {e}", level="error", tag="Backtest")
        df = None
        ...
        for i, row in df.iterrows():
            ...
            if row['target'] == 1:
                ...
                if oid:
                    ...
                    execution_queue.put({'order_id': oid, 'symbol': 'XAUUSD', 'qty': 1, 'price': row['Close']})
            ...
            while not execution_response_queue.empty():
                ...
                if exec_resp['status'] == 'FILLED':
                    omsmm_engine.fill_order(exec_resp['order_id'], fill_qty=exec_resp['fill_qty'])
