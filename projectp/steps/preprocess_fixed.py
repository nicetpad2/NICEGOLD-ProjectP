...
try:
    ...
    print("✅ Evidently successfully imported")
        ...
        with Progress(SpinnerColumn(), BarColumn(), TextColumn("[progress.description]{task.description}"), TimeElapsedColumn(), console = console) as progress:
            ...
            if config and "data" in config and config["data"].get("auto_thai_csv"):
                ...
                for df_next in dfs[1:]:
                    df_merged = pd.merge(df_merged, df_next, on = key, how = 'outer', suffixes = ("", "_tf2"))
                df = df_merged
            ...
            progress.update(task, advance = 15, description = "[cyan]Normalizing column names...")
            col_map = {}
            for c in df.columns:
                cl = c.lower()
            ...
            datetime_cols = [col for col in df.columns if col.lower() in ["date", "datetime", "timestamp", "time"]]
            main_cols = ["Open", "High", "Low", "Close", "Volume"]
            exclude_cols = ["target"] + datetime_cols + main_cols
            df, dropped_corr = remove_highly_correlated_features(df, threshold = 0.95, exclude_cols = exclude_cols)
            if dropped_corr:
                pro_log(f"[Preprocess] Dropped highly correlated features: {dropped_corr}", tag = "Preprocess")

            # Select final columns
            features = ["Open", "High", "Low", "Close", "Volume", "ma5", "ma10", "returns", "volatility", "momentum", "rsi", "macd"]
            target = "target"
            ...
            if df.isnull().sum().sum() > 0:
                df = df.ffill().bfill()
                pro_log(f"[Preprocess] Filled missing values using ffill/bfill.", tag = "Preprocess")

            df = df.dropna(subset = [c for c in features if c in df.columns] + [target])
            df_out = df[keep_cols].copy()
            pro_log(f"[Preprocess] After feature engineering and cleaning: {df_out.shape}", tag = "Preprocess")

            # Schema validation
            progress.update(task, advance = 15, description = "[cyan]Validating data schema...")
            try:
                if PANDERA_AVAILABLE and schema is not None:
                    schema.validate(df_out[expected_cols])
                    pro_log("[Preprocess] Pandera schema validation PASSED.", tag = "Preprocess")
                else:
                    pro_log("[Preprocess] Pandera not available, skipping schema validation.", tag = "Preprocess")
            except Exception as e:
                pro_log(f"[Preprocess] Schema validation failed: {e}", level = "error", tag = "Preprocess")
            ...
            # Save outputs
            progress.update(task, advance = 15, description = "[cyan]Saving processed data...")
            os.makedirs("output_default", exist_ok = True)
            out_path = os.path.abspath(os.path.join("output_default", "preprocessed_super.parquet"))
            df_out.to_parquet(out_path, index = False)

            out_csv_path = os.path.abspath(os.path.join("output_default", "preprocessed.csv"))
            df_out.to_csv(out_csv_path, index = False)
            pro_log(f"[Preprocess] Saved feature engineered data to {out_csv_path}", level = "success", tag = "Preprocess")

            # Export logs and metrics
            pro_log_json({"event": "end_preprocess", "result": str(out_path)}, tag = "Preprocess", level = "SUCCESS")
            ...
            resource_log_path = os.path.join("output_default", "preprocess_resource_log.json")
            with open(resource_log_path, "w", encoding = "utf - 8") as f:
                json.dump(resource_info, f, indent = 2)
            pro_log(f"[Preprocess] Resource log exported to {resource_log_path}", tag = "Preprocess")
            ...
            try:
                ...
                summary_path = os.path.join("output_default", "preprocess_summary_metrics.json")
                with open(summary_path, "w", encoding = "utf - 8") as f:
                    json.dump(summary, f, indent = 2)
                pro_log(f"[Preprocess] Summary metrics exported to {summary_path}", tag = "Preprocess")
            except Exception as e:
                pro_log(f"[Preprocess] Summary metrics export error: {e}", level = "warn", tag = "Preprocess")

            progress.update(task, completed = 100, description = "[green]Preprocessing completed successfully!")

        # Final summary
        if RICH_AVAILABLE:
            summary_table = Table(title = "[bold green]สรุปผล Preprocess", show_header = True, header_style = "bold magenta")
            ...
            console.print(Panel(summary_table, title = "[bold blue]✅ Preprocess เสร็จสมบูรณ์!", border_style = "bright_green"))

        processing_metrics["status"] = "completed"
        processing_metrics["end_time"] = time.time()
        processing_metrics["duration"] = processing_metrics["end_time"] - processing_metrics["start_time"]
