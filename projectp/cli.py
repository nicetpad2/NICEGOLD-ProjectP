...
from projectp.pipeline import run_debug_full_pipeline, run_full_pipeline
        from projectp.steps.preprocess import run_preprocess
        from sklearn.ensemble import RandomForestClassifier
from src.features.ml_auto_builders import build_lgbm_model, build_xgb_model
import argparse
...
try:
    ...
    # TensorFlow not installed - this is optional
    pass
except Exception as e:
    print("TensorFlow: Failed to configure GPU settings:", str(e))
...
def load_model_config(config_path = "config.yaml"):
    ...
    elif model_name.lower().startswith("lgb") or model_name.lower().startswith(
        "lightgbm"
    ):
...
def main_cli():
    ...
    parser.add_argument(
        ...
        default = None, 
        help = "เลือกโหมด pipeline", 
    )
    parser.add_argument(
        " -  - env", choices = ["dev", "prod"], default = "dev", help = "Environment config"
    )
    parser.add_argument(
        " -  - auto", action = "store_true", help = "Run in auto mode without interactive menu"
    )
    ...
    if not auto_mode:
        ...
        print(
            " 2. debug_full_pipeline    - ดีบัค: ตรวจสอบทุกจุดของ full_pipeline (log ละเอียด, ไม่หยุดเมื่อ error)"
        )
        print(
            " 3. preprocess              - เตรียมข้อมูล features (สร้างไฟล์ features_main.json/preprocessed_super.parquet)"
        )
        print(
            " 4. realistic_backtest      - แบลคเทสเสมือนจริง (เทพ, ใช้ข้อมูลจาก full pipeline, walk - forward)"
        )
        print(
            " 5. robust_backtest          - แบลคเทสเทพ (walk - forward, parallel, เลือกโมเดลได้)"
        )
        print(
            " 6. realistic_backtest_live  - แบลคเทสเหมือนเทรดจริง (train เฉพาะอดีต, test อนาคต, save model)"
        )
        print(" -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  - ")
    ...
    if not mode and not auto_mode:
        while True:
            ...
            if (
                user_input == ""
                or user_input == "1"
                or user_input.lower() == "full_pipeline"
            ):
                mode = "full_pipeline"
                break
    ...
    if mode == "full_pipeline":
        ...
        if model_choice == "2":
            ...
            print(
                "[Custom] โปรดแก้ไขโค้ดเพื่อ import และกำหนด model_class/model_params เอง"
            )
            return
        ...
        run_realistic_backtest_live_model(
