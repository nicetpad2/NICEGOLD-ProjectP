...
class RealDataValidator:
    ...
    def __init__(self, datacsv_path: str = None):
        ...
        if datacsv_path is None:
            # Default to root datacsv folder (go up one level from projectp)
    ...
    def validate_datacsv_folder(self) -> bool:
        ...
        pro_log(
            f"✅ Found {len(csv_files)} CSV files in datacsv: {csv_files}", 
            tag = "DataValidator", 
        )
        ...
        if not valid_files:
            error_msg = (
                f"❌ CRITICAL: No valid real data files found in {self.datacsv_path}."
            )
            pro_log(error_msg, level = "error", tag = "DataValidator")
            raise ValueError(error_msg)

        pro_log(
            f"✅ Validated {len(valid_files)} real data files: {valid_files}", 
            tag = "DataValidator", 
        )
        return True
    ...
    def _validate_csv_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            if file_size < 1024:  # Less than 1KB is suspicious
                pro_log(
                    f"⚠️ WARNING: {os.path.basename(file_path)} is very small ({file_size} bytes)", 
                    tag = "DataValidator", 
                )
                return False
            ...
            if len(df) < 100:
                pro_log(
                    f"⚠️ WARNING: {os.path.basename(file_path)} has only {len(df)} rows", 
                    tag = "DataValidator", 
                )
                return False
            ...
            if required_found < 4:
                pro_log(
                    f"⚠️ WARNING: {os.path.basename(file_path)} missing required OHLC columns", 
                    tag = "DataValidator", 
                )
                return False
            ...
            for col in numeric_cols:
                if df[col].nunique() == 1:  # All values the same
                    pro_log(
                        f"⚠️ WARNING: {os.path.basename(file_path)} column {col} has constant values (dummy data?)", 
                        tag = "DataValidator", 
                    )
                    return False

            pro_log(
                f"✅ {os.path.basename(file_path)} validated as real data", 
                tag = "DataValidator", 
            )
            return True
            ...
            pro_log(
                f"❌ ERROR validating {os.path.basename(file_path)}: {e}", 
                level = "error", 
                tag = "DataValidator", 
            )
            return False
    ...
    def load_real_data(self, filename: str = None) -> pd.DataFrame:
        ...
        try:
            df = pd.read_csv(file_path)
            pro_log(
                f"✅ Loaded real data: {df.shape} from {os.path.basename(file_path)}", 
                tag = "DataValidator", 
            )

            # Log data summary for transparency
            pro_log(
                f"Data range: {len(df)} rows, columns: {list(df.columns)}", 
                tag = "DataValidator", 
            )
...
def enforce_real_data_only():
    ...
    pro_log(
        "🛡️ Real data enforcement activated - only datacsv data allowed", 
        tag = "DataValidator", 
    )
    return validator