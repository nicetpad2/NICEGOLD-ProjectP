...
# -*- coding: utf-8 -*-
"""
AI Agents Standalone Runner (Simple Version)
============================================

Standalone script for running AI Agents with graceful error handling.
...
def try_import_agent_controller():
    """Try to import AgentController with error handling."""
    try:
        from agent.agent_controller import AgentController

        return AgentController, True
    except ImportError as e:
        print(f"⚠️  Warning: AgentController not available ({e})")
        print("   Some features may not work, but web interface should be available.")
        return None, False
...
def run_analyze_action(project_root: str, verbose: bool = False) -> dict:
    ...
    try:
        AgentController, available = try_import_agent_controller()
        if not available:
            return {
...
def run_web_action(port: int = 8501, verbose: bool = False) -> dict:
    ...
    try:
        ...
        # Try enhanced web interface first, then fallback to basic
        web_app_paths = [
            os.path.join(PROJECT_ROOT, "ai_agents_web_enhanced.py"),
            os.path.join(PROJECT_ROOT, "ai_agents_web.py"),
        ]
        ...
        if not web_app_path:
            return {
                "error": "No web interface file found. Please check installation.",
                "action": "web",
            }

        if verbose:
            print(f"🚀 Launching web dashboard at http://localhost:{port}")
            print(f"   Using: {os.path.basename(web_app_path)}")
