...
class EmergencyFixManager:
        ...
        try:
            ...
            if len(feature_cols) > 1:
                ...
                if max_target_corr < 0.05:
                    issues.append(
                        f"Very low feature - target correlation: {max_target_corr:.4f}"
                    )
        ...
        try:
            ...
            if target_col not in df_fixed.columns:
                ...
                if len(df_fixed.columns) > 0:
                    ...
                    if len(numeric_cols) > 0:
                        ...
                        df_fixed[target_col] = (df_fixed[base_col] > median_val).astype(
                            int
                        )
                        self.log_fix(
                            f"Created target from {base_col} median split", "INFO"
                        )
                    else:
                        # สร้าง random target
                        np.random.seed(42)
                        df_fixed[target_col] = np.random.choice(
                            [0, 1], size=len(df_fixed), p=[0.7, 0.3]
                        )
            ...
            for col in feature_cols:
                if df_fixed[col].dtype in ["int64", "float64"]:
                    ...
                    df_fixed[col] = df_fixed[col].replace(
                        [np.inf, -np.inf], df_fixed[col].median()
                    )
            ...
            if len(numeric_cols) >= 2:
                ...
                for col in numeric_cols[:3]:
                    df_fixed[f"{col}_ma3"] = (
                        df_fixed[col].rolling(3, min_periods=1).mean()
                    )

                # Interaction features
                df_fixed["feature_sum"] = (
                    df_fixed[numeric_cols[0]] + df_fixed[numeric_cols[1]]
                )
                df_fixed["feature_ratio"] = df_fixed[numeric_cols[0]] / (
                    df_fixed[numeric_cols[1]] + 1e-8
                )
            ...
            if len(target_counts) >= 2:
                ...
                if class_ratio > 50:  # Extreme imbalance
                    self.log_fix(
                        f"Fixing extreme class imbalance: {class_ratio:.1f}:1", "WARN"
                    )
    ...
    def _balance_classes(self, df: pd.DataFrame, target_col: str) -> pd.DataFrame:
        ...
        try:
            ...
            result_df = pd.concat(balanced_dfs, ignore_index=True).sample(
                frac=1, random_state=42
            )
            self.log_fix(
                f"Balanced classes: {result_df[target_col].value_counts().to_dict()}",
                "INFO",
            )
    ...
    def validate_and_fix_config(self, config_path: str = "config.yaml") -> bool:
        ...
        try:
            ...
            complete_config = {
                ...
                "parallel": {"enabled": True, "n_jobs": -1, "backend": "threading"},
                "visualization": {
                    "enabled": True,
                    "show_plots": False,
                    "save_plots": True,
            ...
            if config_file.exists():
                try:
                    ...
                    self.log_fix(
                        f"Loaded existing config: {len(current_config)} sections"
                    )
                except Exception as e:
            ...
            def check_and_add_field(config_dict, key, default_value, path=""):
                ...
                if key not in config_dict:
                    ...
                    for sub_key, sub_value in default_value.items():
                        if check_and_add_field(
                            config_dict[key], sub_key, sub_value, full_path
                        ):
                            changed = True
                    return changed
                return False
            ...
            if config_changed or missing_fields:
                ...
                self.log_fix(
                    f"Updated config with {len(missing_fields)} missing fields:"
                )
...
def ensure_complete_config():
    ...
    default_config = {
        "data": {"source": "dummy_m1.csv", "target_column": "target"},
        "model": {
            "features": [
                "Open",
                "Volume",
        ...
        "training": {
            ...
            "random_state": 42,
            "test_size": 0.3,
            "walk_forward": {"enabled": True, "window_size": 1000, "step_size": 100},
        },
        "metrics": {
            "primary": "auc",
...
def create_emergency_fix_manager(
        ...
        for file_path in data_files:
            if Path(file_path).exists():
                try:
                    ...
                    fix_manager.log_fix(
                        f"Loaded data from {file_path}: {df.shape}", "INFO"
                    )
                    break
                except Exception as e:
...
def _create_synthetic_data(fix_manager: EmergencyFixManager) -> pd.DataFrame:
    ...
    try:
        ...
        fix_manager.log_fix(
            f"Target distribution: {df['target'].value_counts().to_dict()}", "INFO"
        )
...
if __name__ == "__main__":
    ...
    logging.info("🔥 Testing Integrated Emergency Fixes...")
    ...
    for mode in modes:
        logging.info(f"🧪 Testing {mode}...")
        success = apply_emergency_fixes_to_pipeline(mode)

        if success:
            logging.info(f"✅ {mode} emergency fixes completed")
        else:
            logging.error(f"❌ {mode} emergency fixes failed")

    logging.info("🎉 Integration testing completed!")
